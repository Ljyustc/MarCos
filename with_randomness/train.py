"""
Two-phase training (with randomness factor) for the MarCos model.

All hyper-parameters are exposed as CLI flags so the same script can drive
either the Qwen or Llama backbone, in either phase 1 (main modules) or phase 2
(random predictor). See run_*.sh in this folder for concrete invocations.

Examples:
    torchrun --standalone --nproc_per_node=4 train.py \\
        --backbone qwen --model_path Qwen/Qwen2.5-0.5B \\
        --phase 1 --step 3 --max_iters 200 \\
        --train_data train_data/train.json --val_data train_data/valid.json
"""
import argparse
import os
import time
...
def parse_args():
    ...
    p.add_argument('--model_path', default=None,
                   help='HF id or local path; defaults to the canonical id for --backbone')
    # Phase / init
    p.add_argument('--phase', choices=['1', '2'], default='1',
                   help="'1' trains the main modules; '2' freezes them and trains the random predictor")
    p.add_argument('--init', choices=['pretrained', 'config', 'resume'], default='pretrained',
                   help="'pretrained' = load backbone weights, 'config' = random init from backbone arch, 'resume' = load --resume_ckpt")
    p.add_argument('--resume_ckpt', default=None, help='Checkpoint path required when --init=resume (typical for phase 2)')
    # Architecture / dimensions
    p.add_argument('--step', type=int, default=3, help='Number of thinking steps to supervise')
    p.add_argument('--num_iterations', type=int, default=3)
    p.add_argument('--neuron_dim_t', type=int, default=10)
    ...
    p.add_argument('--random_dim', type=int, default=-1,
                   help='Latent dim of the random predictor; -1 = neuron_dim_r * hidden_size')
    # Optimization
    ...
    p.add_argument('--lr_decay_iters', type=int, default=-1, help='-1 = use --max_iters')
    ...
    # Sparsity / DWA controller
    p.add_argument('--L1_weight', type=float, default=1e-4)
    p.add_argument('--L1_TARGET', type=float, default=10.0)
    ...
    p.add_argument('--wandb_project', default='marcos')
...
def compute_loss(model, batch, phase, step, L1_weight, device):
    ...
    # phase == '2': only the random-predictor loss
...
class DWAController:
    def __init__(self, target=5.0, update_freq=50, init_weight=1e-3, min_w=1e-6, max_w=0.1):
        self.target = target
...
def main():
    ...
    if args.init == 'resume':
        ...
        print(f"Initializing from architecture config (random weights), phase={args.phase}")
        ...
        print(f"Initializing from pretrained weights at {args.model_path}, phase={args.phase}")
        model = ModelM(tokenizer, model_path=args.model_path,
    ...
    if args.init == 'resume' and checkpoint is not None and 'optimizer' in checkpoint:
    ...
    dwa_controller = DWAController(
        ...
        init_weight=args.L1_weight, min_w=1e-6, max_w=0.1,
    )
    ...
    best_val_loss = 1e9
    ckpt_prefix = f"ckpt_{time.strftime('%Y%m%d_%H%M%S')}_ddp_{args.task}_{args.backbone}_phase{args.phase}_step{args.step}"
    ...
    for iter_num in range(args.max_iters):
        ...
        if iter_num % args.eval_interval == 0 and master_process:
            ...
            with torch.no_grad(), ctx:
                ...
                for batch in val_loader:
                    nll, tokens, spa = compute_loss(model, batch, args.phase, args.step, l1_weight, device)
                    total_nll += nll
                    total_tokens += tokens
                    total_sparsity += spa
            val_loss = total_nll / total_tokens
            spa_loss = total_sparsity / total_tokens if total_tokens else 0
            print(f"step {iter_num}: val loss {val_loss:.4f}, spar loss {spa_loss:.4f}")
            if args.wandb_log:
                import wandb
                wandb.log({"iter": iter_num, "val/loss": float(val_loss), "lr": lr})
        ...
        for batch in train_loader:
            ...
            with ctx:
                loss, tokens, spa_loss = compute_loss(model, batch, args.phase, args.step, l1_weight, device)
        ...
        if iter_num % args.log_interval == 0 and master_process:
            denom = train_count if train_count else 1
            print(f"iter {iter_num}: loss {train_loss/denom:.4f}, spar {spar_train_loss/denom:.4f}, time {dt:.3f}s")
